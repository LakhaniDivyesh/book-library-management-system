...
import { adminLogin } from '../../services/home.service';
...
function AdminLogin() {
    ...
    const onSubmit = (value) => {
        ...
        adminLogin(data).then((r) => {
            if (r?.data.length > 0 && r?.code === '1') {
                localStorage.setItem('user', JSON.stringify(r.data));
                toast.success(`Welcome! ${r.data[0].name}`, {
    ...
    return (
        <>
            <div className="row my-0 mx-0 my-5 p-0 d-flex justify-content-center">
                <ToastContainer />
                <div className='col-md-4 bg-white p-5 form-con'>
                    <div className="row m-0 row-con">
                        <h3 className='h3 text-center w-100 mb-5'>Admin Login</h3>
